...
public class EscapeMaze {
  ...
  public String solution(int maxX, int maxY, int startX, int startY, int endX, int endY,
      int countMoves) {
    // 불가능한 케이스 처리
  ...
  private void updateRouteIfNeeded(LinkedList<String> routeList, int countMoves, int maxX,
      int maxY) {
...
//        int currentY = startY;
  ...
  private String getRouteString(char moveChar, int countMoves) {
    return IntStream.range(0, countMoves).mapToObj(i -> String.valueOf(moveChar))
