package programmers.programmers.kakao.blind_recruitment_23;
...
public class ExpressibleBinaryTree {
    ...
    public int[] solution(long[] numbers) {
        ...
        for (int i = 0; i < numbers.length; i++) {
            expressibleNumbers[i] = isExpressibleByBinaryTree(numbers[i]) ? EXPRESSIBLE_NUMBER : INEXPRESSIBLE_NUMBER;
        }
    ...
    private boolean isExpressibleByBinaryTree(long number) {
        ...
        if (paddedBinaryNumber.charAt(rootIndex) == DUMMY_NODE_NUMBER && paddedBinaryNumber.length() > 1) {
            return false;
        }
